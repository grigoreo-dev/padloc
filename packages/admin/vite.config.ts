    ...
    return {
        ...
        transformIndexHtml(html, context) {
            ...
            if (context.server) {
                ...
                const devWebsocketUrl = websocketOriginFor(adminUrl, Number(process.env.PL_ADMIN_PORT || 9090));
                content = content
        ...
        closeBundle() {
            ...
            html = html.replace("[REPLACE_CONNECT]", adminUrl.startsWith("http://localhost") ? adminUrl : "");
...
export default defineConfig({
    ...
    server: {
        host: "0.0.0.0",
        port: Number(process.env.PL_ADMIN_PORT || 9090),
    },
    plugins: [
        padlocAssetsPlugin(),
        VitePWA({
            ...
            manifest: {
                ...
                icons: [{ src: `${adminUrlPath}app-icon.png`, sizes: "512x512", type: "image/png" }],
            },
        }),
        padlocCspPlugin(adminUrl, serverUrl),
    ],
});
